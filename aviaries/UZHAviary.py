...
dummy_text = lambda txt, client_id: p.addUserDebugText(
    txt,
    txt_position,
    lifeTime=0,
    textSize=txt_size,
...
refreshed_text = lambda txt, client_id, replace_id: p.addUserDebugText(
    txt,
    txt_position,
    lifeTime=0,
    textSize=txt_size,
...
class UZHAviary(BaseRLAviary):
    ...
    def __init__(
        ...
        initial_xyzs: np.ndarray = np.array([[0.0, 0.0, 0.1125]]),
        initial_rpys=None,
        ...
        # TRAJECTORY
        self.WAYPOINT_BUFFER_SIZE = (
            waypoint_buffer_size  # how many steps into future to interpolate
        )
        self.trajectory = self.set_trajectory()
        assert self.WAYPOINT_BUFFER_SIZE < len(
            self.trajectory
        ), "Buffer size should be smaller than the number of waypoints"
        self.current_waypoint_idx = 0
        self.furthest_reached_waypoint_idx = 0
        self.future_waypoints_relative = (
            self.trajectory[
                self.current_waypoint_idx : self.current_waypoint_idx
                + self.WAYPOINT_BUFFER_SIZE
            ]
        ...
        self.projection_id = p.addUserDebugLine(
            drone_pos, drone_pos, [0.3, 0.3, 0.3], physicsClientId=self.CLIENT
        )
        self.waypoint_connection_ids = [
            p.addUserDebugLine(
                drone_pos, drone_pos, [0, 0, 0], physicsClientId=self.CLIENT
            )
            for i in range(self.WAYPOINT_BUFFER_SIZE)
        ]
    ...
    def _computeReward(self):
        ...
        self.current_projection, self.current_projection_idx, reached_distance = (
            self.rewards.get_travelled_distance(drone_pos)
        )
    ...
    def _computeTerminated(self):
        if self.eval_mode:
            ...
            is_close = (np.linalg.norm(drone_pos - self.trajectory[-1]) < 0.2) and (
                velocity_norm < 0.05
            )
            if is_close:
                self.reset_vars()
    ...
    def _observationSpace(self):
        ...
        obs_lower_bound = np.hstack(
            [
                obs_lower_bound,
                np.array(
                    [[lo, lo, lo] for i in range(self.WAYPOINT_BUFFER_SIZE + 1)]
                ).reshape(1, -1),
            ]
        )
        obs_upper_bound = np.hstack(
            [
                obs_upper_bound,
                np.array(
                    [[hi, hi, hi] for i in range(self.WAYPOINT_BUFFER_SIZE + 1)]
                ).reshape(1, -1),
            ]
        )
    ...
    def step(self, action):
        ...
        if self.GUI and not self.visualised:
            ...
            self.projection_id = p.addUserDebugLine(
                drone, drone, [1, 0, 0], physicsClientId=self.CLIENT
            )
            ...
            self.projection_id = p.addUserDebugLine(
                ...
                [1, 0, 0],
                physicsClientId=self.CLIENT,
                replaceItemUniqueId=self.projection_id,
            )
            # print(self.future_waypoints_relative)
            for i in range(len(self.waypoint_connection_ids)):
                self.waypoint_connection_ids[i] = p.addUserDebugLine(
                    drone_pos,
                    self.future_waypoints_relative[i],
                    [0.3, 0.3, 0.3],
                    physicsClientId=self.CLIENT,
                    replaceItemUniqueId=self.waypoint_connection_ids[i],
                )

            self.text_id = refreshed_text(
                str(self.rewards.cur_reward), self.CLIENT, self.text_id
            )
    ...
    def _computeObs(self):
        ...
        ret = (
            np.hstack([obs[2], obs[7:10], obs[10:13], obs[13:16]])
            .reshape(1, -1)
            .astype("float32")
        )
        self.future_waypoints_relative = self.trajectory[
            self.furthest_reached_waypoint_idx : self.furthest_reached_waypoint_idx
            + self.WAYPOINT_BUFFER_SIZE
        ]
        ...
        ret = np.hstack(
            [
                ret,
                self.current_projection - obs[:3].reshape(1, -1).astype("float32"),
                (self.future_waypoints_relative - obs[:3])
                .reshape(1, -1)
                .astype("float32"),
            ]
        )
